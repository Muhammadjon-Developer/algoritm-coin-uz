...
const Saidbar = () => {
  ...
  return (
    <div className={darkMode ? "col-span-1 row-span-9 bg-slate-800 border-r border-slate-500 shadow-sm text-slate-200" : "col-span-1 row-span-9 shadow-xl bg-white  text-slate-600"}>
      ...
      <div className="py-8 px-4 h-[60%]">
        {
          dataMenu.map(item => (
            <Link to={item.link}>
            <div key={item.title} className={darkMode ? `${styles.flexStart} gap-2 hover:bg-slate-600 ${styles.roundedEl}}` : `${styles.flexStart} gap-2 hover:bg-slate-300 ${styles.roundedEl}`}>
              {<item.icon />}
      ...
      <div className="px-4">
        <div className={`${styles.flexCenter} gap-4 bg-indigo-700 rounded-md py-2`}>
          {user
